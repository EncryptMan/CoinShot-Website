        ...
        <Card className="w-full p-3 bg-slate-600">
            <Link href={href}>
                <div className="flex gap-5 max-lg:flex-col">
                    <Image width={200} height={200} src={image} alt={`${title} blog icon`} className="rounded-lg overflow-hidden max-lg:mx-auto" />
                    <div className="flex-grow">
                        <span className="capitalize text-blue-400">{category}</span>
                        <div className="flex items-center justify-between">
                            <h2 className="text-2xl font-semibold max-md:text-xl">{title}</h2>
                        </div>
